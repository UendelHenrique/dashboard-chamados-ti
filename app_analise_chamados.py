...
def carregar_dados(arquivos):
    ...
    if linhas_removidas > 0:
        st.warning(f"{linhas_removidas} linha(s) foram removidas por conterem um formato de data inválido.")
...
try:
    ...
    min_val = df_dados['Data criação'].min()
    max_val = df_dados['Data criação'].max()
...
with tab1:
    ...
    fig = px.bar(
        tempo_por_categoria, x='Tempo Resolvido (h)', y='Categoria', orientation='h',
